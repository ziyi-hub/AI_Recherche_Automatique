# ################################################################
# ## 2-3 两次迭代 vs 不限迭代次数，直到损失函数不再下降
# #################################################################
# print("################################################################")
# print("2-3 两次迭代 vs 不限迭代次数，直到损失函数不再下降")
# print("################################################################")
## 迭代次数的更改可以通过改变epoch_num实现
## 损失函数不再下降，即如果训练集loss连续几次下降不明显，就早停。一个简单的策略可以如下：
## 维护一个list，相邻元素依次作差，差值较小且均小于某一个设定的阈值
## 添加内容 ## 7 定义判断损失函数不再下降的辅助函数   8-2中定义loss_list  8-3中使用loss_list
...
## 7 定义判断损失函数不再下降的辅助函数
def not_drop_check(input_list, threshold):
    for i in range(len(input_list) -1):
        if(input_list[i] - input_list[i+1]) > threshold:
            return False      # 仍在下降
    return True             # 停止下降
...
for epoch in range(epoch_nums):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        if(len(loss_list) >= 5):
            loss_list.pop(0)
            if (not_drop_check(input_list = loss_list, threshold=0.001)):
                stop_early = True
    if(epoch == 0):
        epoch_train_ops = batch_train_ops * iteration_num
    if(stop_early):
        print("early stop, early stop, early stop")
        break
