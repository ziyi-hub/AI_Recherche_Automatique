# ################################################################
# ## 2-2 插入一个额外的层，可以是卷积层，也可以是全连接层
# #################################################################
# print("################################################################")
# print("2-2 插入一个额外的层，可以是卷积层，也可以是全连接层")
# print("################################################################")
## 修改位置在 # 4-1 定义模型 插入一个卷积, 同时统计卷积的函数count_total_operations
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(32 * 1 * 1, 120)  # 全连接层的输入需做相应的调整
    ...
    def count_operations(self, x):
        ...
        conv2_out_pooled = self.pool(F.relu(conv2_out))  # Apply max pooling after the second convolution
        conv3_out = self.conv3(conv2_out_pooled)
        conv3_out_pooled = self.pool(F.relu(conv3_out))  # Apply max pooling after the third convolution        
        ...
        # Count operations for convolutional layer 3
        conv3_ops = self.count_conv_operations(conv2_out_pooled, conv3_out, conv3_out_pooled, self.conv3, self.pool)        
        return conv1_ops, conv2_ops, conv3_ops      
    ...
    def count_total_operations(self, x):
        conv1_ops, conv2_ops, conv3_ops = self.count_operations(x)
        ...
        total_ops = sum(op[3] for op in [conv1_ops, conv2_ops, conv3_ops, fc1_ops, fc2_ops, fc3_ops])
        return total_ops
# 4-2 实例化网络
net = Net().to(device)
